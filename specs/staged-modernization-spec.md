...
- 2026-04-10 | Stage 9 Isolation Test 1 | Flipped only `compilerOptions.target` (`ES5` -> `ES6`) on the known-good branch; validation tag `v13.0.2-es5.12`; user-confirmed legacy device/cast smoke test pass | PASS | Revert test commit `7e6405c` if needed | Continue isolation with `moduleResolution`.
- 2026-04-10 | Stage 9 Isolation Test 2 | Flipped only `compilerOptions.moduleResolution` (`node` -> `bundler`) with all other working settings retained (including ES6 target from Test 1); validation tag `v13.0.2-es5.13`; user-confirmed legacy device/cast smoke test fail | FAIL (CONFIRMED BREAKING CHANGE) | Reverted via `5b07f3c` | Root cause isolated: `moduleResolution: bundler` breaks legacy/cast path in this project build pipeline.
- 2026-04-10 | Stage 9 Isolation Test 3 | Upgraded only `typescript` (`5.9.3` -> `6.0.2`) while retaining known-good runtime settings (`target: ES6`, `moduleResolution: node`) and existing build plugin (`rollup-plugin-ts`); added `ignoreDeprecations: "6.0"` to keep TS6 compile-compatible with current config. Validation passed (`corepack pnpm test:ts`, `corepack pnpm build`, `corepack pnpm lint` with expected unsupported-version warning from `@typescript-eslint`), published validation tag `v13.0.2-es5.14`; user-confirmed legacy device/cast smoke test pass | PASS | Revert Test 3 commit(s) if needed | Continue isolation with plugin migration.
- 2026-04-10 | Stage 9 Isolation Test 4 | Switched only Rollup TypeScript plugin from `rollup-plugin-ts` to `@rollup/plugin-typescript` (updated `rollup.config.js` plugin import/options and devDependencies) while retaining known-good runtime settings (`target: ES6`, `moduleResolution: node`) and `typescript` 6.0.2 from Test 3; validation passed (`corepack pnpm test:ts`, `corepack pnpm build`, `corepack pnpm lint` with expected unsupported-version warning from `@typescript-eslint`), published validation tag `v13.0.2-es5.15` | IN PROGRESS | Revert Test 4 commit(s) if needed | Run legacy device/cast smoke test for this tag.
